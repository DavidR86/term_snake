...
echo -e "The game will now be compiled. You can compile it yourself by running 'cd ./src/ && gcc -o ../snake_ascii_ver snake_ascii_version.s -no-pie && cd ..'"
cd ./src && gcc -o ../snake_ascii_ver snake_ascii_version.s -no-pie && cd ../
echo -e "Done! Now you should be able to play the game by writing './snake_ascii_ver' from withing the term_snake folder \nPro-tip: You can customize your terminal to make the game look different ( by changing the size of the window, the fonts, colors and even the background ) \n\033[0m"
